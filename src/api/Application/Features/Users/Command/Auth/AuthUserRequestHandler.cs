...
{
    /// <summary>
    /// AuthUserRequestHandler is responsible for handling the AuthUserRequest command, which authenticates a user based on their email and password. It interacts with the IUserRepository to retrieve user information based on the provided email, uses IPasswordHasher to verify the provided password against the stored password hash, and utilizes ITokenService to generate access and refresh tokens upon successful authentication. The handler also stores the generated refresh token in the IRefreshTokenStore for future token refresh operations. When executed, this handler will return an AuthResponse containing the access token and refresh token if authentication is successful; otherwise, it will throw an exception indicating the failure reason (e.g., user not found or incorrect password).
    /// </summary>
